...
import (
	...
	redisclusterv1alpha1 "github.com/michaelhenkel/tungstenfabric-operator/pkg/apis/tf/v1alpha1"
...
func (r *ReconcileRedisCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		dep := r.deploymentForRedisCluster(instance)
	...
	// List the pods for this rediscluster's deployment
	podList := &corev1.PodList{}
	labelSelector := labels.SelectorFromSet(labelsForRedisCluster(instance.Name))
	...
	if err != nil {
		reqLogger.Error(err, "Failed to list pods.", "RedisCluster.Namespace", instance.Namespace, "RedisCluster.Name", instance.Name)
		return reconcile.Result{}, err
	}
	...
	if !reflect.DeepEqual(podNames, instance.Status.Nodes) {
		...
		if err != nil {
			reqLogger.Error(err, "Failed to update RedisCluster status.")
			return reconcile.Result{}, err
		}
	}
	...
	if int32(len(podIpList)) == size && initContainerRunning {
		...
		err = r.client.Get(context.TODO(), types.NamespacedName{Name: "tfredisclustercmv1", Namespace: foundConfigmap.Namespace}, foundConfigmap)
		if err != nil && errors.IsNotFound(err) {
			...
			*/
			cm := r.configmapForRedisCluster(instance, podIpList)
...
func newPodForCR(cr *redisclusterv1alpha1.RedisCluster) *corev1.Pod {
...
func (r *ReconcileRedisCluster) configmapForRedisCluster(m *redisclusterv1alpha1.RedisCluster, podIpList []string) *corev1.ConfigMap {
	nodeListString := strings.Join(podIpList,",")
	configMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name: "tfredisclustercmv1",
...
// deploymentForRedisCluster returns a rediscluster Deployment object
func (r *ReconcileRedisCluster) deploymentForRedisCluster(m *redisclusterv1alpha1.RedisCluster) *appsv1.Deployment {
	ls := labelsForRedisCluster(m.Name)
	replicas := m.Spec.Size
	redisclusterImage := m.Spec.Image
...
// labelsForRedisCluster returns the labels for selecting the resources
// belonging to the given rediscluster CR name.
